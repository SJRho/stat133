...
- Please note that if you submit the incorrect files, you will get no credit.
...
```{r}
# your code
select(filter(starwars, species == "Human" & sex == "female"), name, homeworld)
```
...
# your code
ggplot(data = fem) +
  geom_text(aes(x = height, y = mass, label = name))
```
...
```{r}
# your code
ggplot(data = filter(starwars, sex == "female")) +
       geom_label(aes(x = height, y = mass, label = name))
```
